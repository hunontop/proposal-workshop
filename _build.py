...
SESSIONS = [
    ...
    ("2편", "AI는 어떻게 답하나 · 14~20장", [
        ("프롬프트 4요소 카드", "files/프롬프트_4요소_카드.md",
         "18장 「잘 묻는 법 · 네 가지만」. 맥락·역할·예시·제약 한 장 요약 + 빈칸 양식. 같은 요청을 한 줄로 한 번, 4요소로 한 번 보내는 비포/애프터 실습이 들어 있습니다.", "ready"),
    ]),
    ("3편", "공고 조사와 수작업 우회 · 21~24장", [
        # ⚠ 5단계 지도는 2026-09-01 내렸다 — 덱·노트 어디에서도 이 자료를 안내하지 않는다.
        #   파일(files/5단계_지도.md)은 남겨 뒀다. 다시 안내할 자리가 생기면 되살린다.
        ("첫 실습 프롬프트 — 공고 조사", "files/프롬프트_공고조사_첫실습.md",
         "21장. 과정에서 처음 직접 시켜 보는 실습입니다. 공고를 찾는 문장과 첨부를 열어 보라는 문장 둘을 복사해서 바로 붙여넣으세요.", "ready"),
    ]),
    ("5편", "에이전트 첫걸음 · 28~32장", [
    ...
    ("7편", "공정 입구 · 선택과 브리프 · 39~43장", [
        ("브리프 예시 — 말벗ON", "files/브리프_예시_말벗ON.md",
         "42장. 공정에 넣는 요구사항 문서가 실제로 어떻게 생겼는지. 없는 값은 비워 둔 형태 그대로입니다.", "ready"),
    ]),
    ("9편", "도식 · 말로 먼저 그린다 · 46~48장", [
        ("도식 5항목 판정표", "files/도식_5항목_판정표.md",
         "46~48장. 도식 후보를 순서·위계·누락·산출물 분리·밀도로 판정하는 표. 세 동사로 묻는 법과 napkin 주의 두 가지도 함께.", "ready"),
    ]),
    ("10편", "디자인과 이미지 프롬프트 · 49~52장", [
        ("배포 프롬프트 ① 기본 — 3병렬 카드", "files/프롬프트_기본_3병렬카드.md",
         "52장 실습 '같은 프롬프트, 스무 개의 그림'에 쓰는 프롬프트. 실제로 채택된 장의 것입니다.", "ready"),
        ("배포 프롬프트 ② 정직 문법 — 검토요망", "files/프롬프트_정직문법_검토요망.md",
         "52장. 근거가 없는 자리를 어떻게 비워 두는지가 프롬프트까지 흘러온 예. 여유가 있을 때 해 보세요.", "ready"),
    ]),
    ("12편", "회수와 마무리 · 56~61장", [
        ("스킬 3줄 양식", "files/스킬_3줄_양식.md",
         "58장. 이름·언제 쓰나·순서 3줄. 새 대화에서 이름만 불러 같은 결과가 나오는지까지가 완료입니다.", "ready"),
        ("산출물 자기점검", "files/산출물_자기점검.md",
         "59장. 편마다 손에 남아야 하는 것 11칸. 제출하는 곳은 없습니다 — 비어 있는 칸을 스스로 찾는 목록입니다.", "ready"),
    ]),
]
...
<header>
  ...
  <p class="lead">반복업무 1/10로 줄이기 · 실습에 쓰는 양식과 프롬프트</p>
</header>
